...
if($status == 'new')
{
  $sel = $mysqli->query("select * from tbl_order where cid = ".$cid." and o_status='Pending'  and a_status=1 order by id desc");
}
...
{
$sel = $mysqli->query("select * from tbl_order where cid = ".$cid." and rid=".$pid." and (o_status='Completed' or   o_status='Cancelled') and a_status=1 order by id desc");	
}
  if($sel->num_rows != 0)
  {
  $result = array();
